...
- **Stateless instances** — Connection state is stored in the configured storage adapter (DynamoDB, etc.)
- **Graceful shutdown** — Call `di.Delivery.Shutdown()` to drain connections before pod termination
...
- **Health checks** — Pipewave exposes health endpoints via its HTTP mux for Kubernetes liveness/readiness probes
